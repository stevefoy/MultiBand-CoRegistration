...
void ImageGeometry::Undistortion_mat(CameraParam& im_cam, double model_id, double sensor_id, char* FeatSavePP)
{
    ...
    if (model_id == 0) {
        ...
        for (int y = ystart; y < yend; ++y) {
			 // Ensure the loop is being parallelized by printing the thread number and y value
			#pragma omp critical
			{
				std::cout << "Thread " << omp_get_thread_num() << " is processing y = " << y << std::endl;
			}
            for (int x = 1; x <= Cn; ++x) {
                MatrixXd pt(3, 1);
                ...
                for (int count = 1; count <= lastcount; count++) {
                    ...
                    MatrixXd A_mat(2, 2);
                    A_mat(0, 0) = 1 + K1 * r2 + 2 * K2 * r2 * r2 + 3 * K3 * r2 * r2 * r2 + 4 * K4 * r2 * r2 * r2 * r2 + 5 * K5 * r2 * r2 * r2 * r2 * r2;
                    A_mat(0, 1) = 2 * P1 * ydd + 2 * P2 * xdd;
                    A_mat(1, 0) = 2 * P1 * xdd + 2 * P2 * ydd;
                    A_mat(1, 1) = 1 + K1 * r2 + 2 * K2 * r2 * r2 + 3 * K3 * r2 * r2 * r2 + 4 * K4 * r2 * r2 * r2 * r2 + 5 * K5 * r2 * r2 * r2 * r2 * r2;

                    MatrixXd invA_mat = A_mat.inverse();
                    MatrixXd deltaxcap = invA_mat * deltaL;
