...
void SortRows(Matrix<Scalar, rows, cols, options, maxRows, maxCols>& sorted, Matrix<Scalar, rows, cols, options, maxRows, maxCols>& target, int coltofocus) {
    ...
    vector<Matrix<Scalar, 1, cols>> matrixRows;
    for (int i = 0; i < target_temp.rows(); ++i) {
        matrixRows.push_back(target_temp.row(i));
    }

    sort(matrixRows.begin(), matrixRows.end(), compareRows<Matrix<Scalar, 1, cols>>);

    for (int i = 0; i < matrixRows.size(); ++i) {
        sorted.row(i) = matrixRows[i];
    }

    sorted.col(0) = sorted.col(coltofocus);
    sorted.col(coltofocus) = sorted.col(0);
}
...
public:
    ...
    int num_sensors_model0;
    int num_sensors_model1;
    int num_sensors_model2;
    int num_sensors_model3;
    int num_sensors_model4;
    int num_sensors_model6;
...
void PatchMatch_process(MatrixXd& DLeft, string file_1, string file_2);
