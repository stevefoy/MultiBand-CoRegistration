...
{
    // Use sprintf to convert double to string and write to buffer
    int written = snprintf(buffer.data() + offset, buffer.size() - offset, "%.17g", number); // Use "%.17g" for precision

    if (written < 0 || static_cast<size_t>(written) >= buffer.size() - offset)
    {
        throw runtime_error("failed writing");
    }

    buffer[offset + written] = separator;
...
void WriteCSV(const string& csv_filepath, const vector<PCpoint>& points) {
    ...
    constexpr size_t BUFFER_SIZE = 1 << 11; // 2kB
...
void PointCloud(char* FeatSavePP, string ImageNameL, int im1, int im2, CameraParam& im_cam,
    ...
    int im = im1;
    int sensor_id = static_cast<int>(im_cam.sensor_ID(im, 0));
    int model_id = static_cast<int>(im_cam.model_ID(im, 0));
    ...
    A << PSx, 0.0, Cx,
        0.0, -PSy, Cy,
        0.0, 0.0, -f;
    MatrixXd Kmat1 = A.inverse().eval(); // Force evaluation
    Kmat1 = (Kmat1 / abs(Kmat1(2, 2))).eval(); // Force evaluation
    ...
    MatrixXd Pmat1_common = (K_common1 * RT).eval(); // Force evaluation
    RT.block(0, 0, 3, 3) = R_common;
    RT.block(0, 3, 3, 1) = -R_common * Te2i_2;
    MatrixXd Pmat2_common = (K_common2 * RT).eval(); // Force evaluation
    ...
    for (double y1 = 0; y1 < DLeft.rows(); y1++) {
        for (double x1 = 0; x1 < DLeft.cols(); x1++) {
            double disp = DLeft(static_cast<int>(y1), static_cast<int>(x1));
            if (disp != -10 && Mask(static_cast<int>(y1), static_cast<int>(x1)) == 255) {
                double x2 = x1 - disp;
                double y2 = y1;
                ...
                TMP31 = (some * A_mat).lu().solve(some * L_mat).eval(); // Force evaluation
                disttemp = (TMP31 - Te2i_1).eval(); // Force evaluation
